...
ttk.Label(root,text="ID").grid(row=1, column=0, pady=10,padx=10)
...
ttk.Radiobutton(root,text="Male",variable=SpanGender, value="Male").grid(row=2, column=1)
ttk.Radiobutton(root,text="Female",variable=SpanGender, value="Female").grid(row=2, column=2)

TextComments=Text(root,width=30, height=15, font = ('Arial', 16))
TextComments.grid(row=4, column=1,columnspan=2)
ttk.Label(root,text="Residence:").grid(row=3, column=0)

buButton=ttk.Button(root,text="Submit")
buButton.grid(row=5,column=3)
...
def ButtonClick():
    ...
    print("EntryId:{}".format(EntryId.get()))
    print("Gender:{}".format(SpanGender.get()))
    print("Comments:{}".format(TextComments.get(1.0,'end')))
    msg=dbConnect.Add(EntryFullName.get(),EntryId.get(),SpanGender.get(),TextComments.get(1.0,'end'))
